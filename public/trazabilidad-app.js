document.addEventListener('DOMContentLoaded', () => {
    ...
    // Función para generar IDs, ahora en el cliente
    ...
    function createBatchCard(batch, type, parentPath = '') {
        ...
        if (type === 'tostado') {
             if (batch.tipoPerfil) {
                infoHtml += `<p class="text-sm text-stone-500 mt-1">Perfil: <strong>${batch.tipoPerfil}</strong></p>`;
            }
            if (batch.perfilAroma) {
            ...
            try {
                if (isCreating) {
                    ...
                    await api('/api/batches', {
                        method: 'POST',
                        body: JSON.stringify({ tipo: type, parent_id: parentId, data: newData }),
                    });
                } else {
                    await api(`/api/batches/${batchData.id}`, {
                        method: 'PUT',
    ...
    async function generateFormHTML(mode, type, data = {}, availableWeight = 0) {
        ...
        switch(type) {
            ...
            case 'molienda':
                ...
                formFields += createInput('pesoNibs', 'Peso Nibs (kg)', 'number', '', data.pesoNibs);
                formFields += createSelect('productoFinal', 'Producto Final', ['Pasta de cacao', 'Manteca de cacao', 'Otro'], data.productoFinal);
    ...
    async function createPerfilSelectHTML(selectedValue = '') {
        try {
            ...
            return `
                <div>
                    <label for="tipoPerfil" class="block text-sm font-medium text-stone-700 mb-1">Tipo de Perfil Sensorial</label>
                    <div class="flex items-center gap-2">
                        <select id="tipoPerfil" name="tipoPerfil" class="w-full p-3 border border-stone-300 rounded-xl">
                            <option value="">Seleccionar un perfil...</option>
                            ${optionsHTML}
                        </select>
                        <a href="/app/perfiles" target="_blank" class="flex-shrink-0 bg-sky-600 hover:bg-sky-700 text-white font-bold p-3 rounded-xl text-sm" title="Añadir Nuevo Perfil">+</a>
                    </div>
                </div>`;
        } catch (error) {
            return `<div class="text-red-500">Error al cargar perfiles. <a href="/app/perfiles" class="text-sky-600 hover:underline">Ir a Perfiles</a>.</div>`;
        }
    }
    ...
    dashboardView.addEventListener('click', async e => {
        ...
        if (button.classList.contains('qr-btn')) { 
            const url = `http://localhost:3000/trazabilidad.html?lote=${button.dataset.id}`; const qr = qrcode(0, 'L'); qr.addData(url); qr.make();

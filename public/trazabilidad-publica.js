document.addEventListener('DOMContentLoaded', () => {
    ...
    function createSensoryProfileSection(h) {
        ...
        let maridajeHtml = '';
        if (maridajesRecomendados) {
            const allRecs = [
                ...(maridajesRecomendados.cafe || []),
                ...(maridajesRecomendados.vino || [])
            ].sort((a, b) => b.puntuacion - a.puntuacion).slice(0, 4);
    
            maridajeHtml = allRecs.map(rec => {
                ...
                <div class="bg-white p-8 rounded-2xl shadow-xl border max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="pt-6 border-t">
                            <h3 class="font-bold text-xl text-amber-800 mb-3">Maridaje Sugerido</h3>
                            <div class="space-y-3">${maridajeHtml}</div>
                        </div>
                    </div>
